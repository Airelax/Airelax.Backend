    ...
    <div class="col-md-6 col-12 position-relative houses">
      ...
      <div class="px-4 pt-md-3">
        <div class="text-start m-md-4 pt-3">
          <span>{{ rooms.length }}間住宿</span>
          <span v-if="Object.keys($store.state.date).length != 0"> · {{ $store.state.date.start }} - {{ $store.state.date.end }}</span>
          <span v-if="$store.getters.TotalCustomer != 0"> ·{{ $store.getters.TotalCustomer }}位房客</span>
          ...
          <div v-if="$store.state.fullWidth > 768">
            ...
            <div class="d-inline-block">
              <button
                ...
                id="priceDropdown" type="button" class="filters border border-1 bg-white p-2 text-dark btn-white rounded-pill"
              ...
              <div class="dropdown-menu mt-3 px-3" aria-labelledby="priceDropdown">
                <div class="mt-3 d-flex w-100 justify-content-center px-3">
                  <input type="text" placeholder="最低價格" class="ms-1 py-2 border-1" v-model="$store.state.filters.lowPrice">
                  <span class="mx-2 py-2">-</span>
                  <input type="text" placeholder="最高價格" class="ms-1 py-2 border-1" v-model="$store.state.filters.highPrice">
                </div>
                <div class="modal-footer">
                  <button
                    type="button"
                  ...
                  <button type="button" class="btn bg-dark text-white dropdown-item w-25 text-center" @click="getNewSearch">儲存</button>
                </div>
              </div>
            </div>
            <div class="d-inline-block">
              ...
              <div class="offcanvas-body small">
                ...
                <div class="d-inline-block w-100">
                  ...
                  <div class="mt-3 d-flex w-100 justify-content-center align-items-center">
                    <input type="text" placeholder="最低價格" class="ms-1 mt-1 w-75 py-2" v-model="$store.state.filters.lowPrice">
                    <span class="mx-2 mt-1">-</span>
                    <input type="text" placeholder="最高價格" class="ms-1 mt-1 w-75 py-2" v-model="$store.state.filters.highPrice">
                  </div>
                </div>
                <hr />
                <div class="d-inline-block w-100">
        ...
        <div>1 - {{ getPage }} 頁，共 {{ rooms.length }} 間住宿</div>
...
export default {
  ...
  computed: {
    getPage: function () {
      return Math.ceil(
        Number(this.rooms.length) / Number(this.perPageRoomCount)
      );
    },
  },
};
